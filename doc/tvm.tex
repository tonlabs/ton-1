...
\section{#1}\fancyhead[C]{\textsc{Chapter \textbf{\thesection.} #1}}}
\def\mysubsection#1{
...
\section{#1}\fancyhead[C]{\textsc{Appendix \textbf{\thesection.} #1}}}
%
\let\tp=\textit
\let\vr=\textit
...
\author{
\textsc{Nikolai Durov} \\[1ex] % Your name
\and
\textsc{Mikhail Komarov} \\[1ex] % Second author's name
\normalsize \texttt{=nil;} Foundation \\ % Second author's institution
\normalsize \href{mailto:nemo@nil.foundation}{nemo@nil.foundation} % Second author's email address
}
\begin{document}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \begin{itemize}
        ...
        \item {\tt F912} --- {\tt PRVGRTH16} ($s$ -- $s$), computes Groth16 proof of the data bits of a {\em Slice\/}~$s1$.
        If $size(s1) \% 8 != 0$, throws cell underflow exception. The value returned is a {\em Slice\/}~$s2$ prepended with
        32 bit integer $S$ – size of each field member - with a proof, which contains 3 field members of size $S$.
        \item {\tt F913} --- {\tt KGGRTH16} ($c$ -- $s$), generates Groth16 key pair from data bits of a {\em Cell\/}~$c$.
        If $size(c) \% 8 != 0$, throws cell underflow exception. The value returned is a {\em Slice\/}~$s$ prepended with two
        32 bit integers specifying keys lengths with two keys: proving key and verifying key of lengths specified.
        \item {\tt F914} --- {\tt CHKGRTH16} ($c$ -- $s$), verifies the Groth16 proof being emplaced in data bits of a {\em Cell\/}~$c$. If $size(c) \% 8 != 0$, throws cell underflow exception.
...
RET
    ...
    \begin{table}
        \captionsetup{font=footnotesize}{\footnotesize
